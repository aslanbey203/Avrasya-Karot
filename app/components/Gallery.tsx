...
export default function Gallery() {
  ...
  return (
    <>
      <section id="galeri" className="bg-slate-950 py-16 sm:py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          ...
          <div className="mt-10 grid gap-5 sm:mt-14 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
            {images.map((item,index) => <button type="button" aria-label={`${item.title} görselini büyüt`} key={item.image} onClick={() => setSelected(index)} className={`group relative overflow-hidden rounded-3xl border border-slate-800 bg-slate-900 text-left ${index === 0 ? "md:col-span-2" : ""}`}><div className="relative h-72 sm:h-80"><Image src={item.image} alt="" fill sizes={index === 0 ? "(max-width: 767px) calc(100vw - 40px), (max-width: 1023px) calc(100vw - 40px), 66vw" : "(max-width: 767px) calc(100vw - 40px), (max-width: 1023px) 50vw, 33vw"} className="object-cover transition duration-700 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/10 to-transparent"/><h3 className="absolute bottom-0 p-6 text-xl font-black text-white">{item.title}</h3></div></button>)}
          </div>
        </div>
      </section>
